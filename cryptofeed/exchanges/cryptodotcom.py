...
class CryptoDotCom(Feed):
    ...
    rest_endpoints = [RestEndpoint('https://api.crypto.com/exchange/v1', routes=Routes('/public/get-instruments'))]
    ...
    request_limit = 100
    valid_candle_intervals = {'1m', '5m', '15m', '30m', '1h', '2h', '4h', '12h', '1d', '1w', '2w', '1M'}
    ...
    def _parse_symbol_data(cls, data: dict) -> Tuple[Dict, Dict]:
        ...
        for entry in data['result']['data']:
            sym = Symbol(entry['base_ccy'], entry['quote_ccy'])
            info['instrument_type'][sym.normalized] = entry['inst_type']
    ...
    async def _ticker(self, msg: dict, timestamp: float):
        for entry in msg['data']:
            await self.callback(TICKER, Ticker(self.id, self.exchange_symbol_to_std_symbol(entry['i']), Decimal(entry['b']), Decimal(entry['k']), self.timestamp_normalize(entry['t']), raw=entry), timestamp)
    ...
    async def _candle(self, msg: dict, timestamp: float):
        ...
        for entry in msg['data']:
            c = Candle(self.id,
                       self.exchange_symbol_to_std_symbol(msg['instrument_name']),
                       int(entry['t']) / 1000,
                       int(entry['t']) / 1000 + timedelta_str_to_sec(interval) - 1,
                       interval,
                       None,
                       Decimal(entry['o']),
                       Decimal(entry['c']),
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        for entry in msg['data']:
            ...
            self._l2_book[pair].book.bids = {Decimal(price): Decimal(size) for price, size, _ in entry['bids']}
            self._l2_book[pair].book.asks = {Decimal(price): Decimal(size) for price, size, _ in entry['asks']}

            await self.book_callback(L2_BOOK, self._l2_book[pair], timestamp, timestamp=timestamp, raw=entry)
    ...
    async def subscribe(self, conn: AsyncConnection):
        ...
        for chan, symbols in self.subscription.items():
            def sym(chan, symbol):
                ...
                if chan_s == CANDLES:
                    interval = self.candle_interval.upper()
